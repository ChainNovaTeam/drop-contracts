...
abstract contract OperatorFilterer {
    ...
    event OperatorFilterEnabled(bool disabled);

    bool public isOperatorFilter;
    ...
    modifier onlyAllowedOperator(address from) virtual {
        ...
        if (isOperatorFilter && address(operatorFilterRegistry).code.length > 0) {
     ...
     */
    function toggleOperatorFilterEnabled() public virtual onlyOperatorFilterAdmin returns (bool) {
        bool enabled = !isOperatorFilter;
        isOperatorFilter = enabled;
        emit OperatorFilterEnabled(enabled);
        return enabled;
    }
     ...
     */
    function updateOperatorFilterRegistry(address newRegistry, address subscriptionOrRegistrantToCopy, bool subscribe)
        public
        virtual
        onlyOperatorFilterAdmin
    {
        ...
        if (!registry.isRegistered(address(this))) {
            _registerAndSubscribe(registry, subscriptionOrRegistrantToCopy, subscribe);
        }
     ...
     */
    function updateRegistrySubscription(address subscriptionOrRegistrantToCopy, bool subscribe, bool copyEntries)
        public
        virtual
        onlyOperatorFilterAdmin
    {
     ...
     */
    function _isOperatorFilterAdmin(address operator) internal view virtual returns (bool);
    ...
    function _registerAndSubscribe(
        ...
        if (registry.isRegistered(address(this))) {
            if (subscribe) {
                registry.subscribe(address(this), subscriptionOrRegistrantToCopy);
            } else {
                registry.copyEntriesOf(address(this), subscriptionOrRegistrantToCopy);
            }
        } else {
            if (subscribe) {
